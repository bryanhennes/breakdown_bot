...
async def pic(ctx):
    await ctx.send(file=discord.File('jesse.jpg'))
...
async def play(ctx):
    ...
    if voice and voice.is_playing():
        await ctx.send("Something is already playing, either enter .stop to end or .skip to play next breakdown.")
    else:
...
async def find(ctx, file_name):
    ...
    for song in songs:
        new_song = song.split('by')
        if transform(file_name) in new_song[0] or transform(file_name) in new_song[1]:
...
async def check(ctx, search):
    await ctx.send("This is what I found: ")
    found_counter = 0
    for song in songs:
    ...
    if found_counter == 0:
        await ctx.send("Sorry bud, didn't find anything :(")
